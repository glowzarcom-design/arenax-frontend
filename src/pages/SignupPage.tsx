    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-hero">
      <Card className="w-full max-w-md p-8 bg-card/50 backdrop-blur-sm border-2 border-border hover:border-secondary transition-all duration-500 group relative overflow-hidden shadow-lg hover:shadow-[0_0_50px_rgba(168,85,247,0.6)]">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 relative z-10">
          ...
          <div>
            <label className="block text-sm font-medium mb-2 text-foreground">In-Game Name (IGN)</label>
            <Input name="ign" type="text" placeholder="Your gaming name" value={formData.ign} onChange={handleChange} required />
          </div>
          <div>
            <label className="block text-sm font-medium mb-2 text-foreground">Free Fire ID</label>
            <Input name="freeFireId" type="text" placeholder="Your Free Fire ID" value={formData.freeFireId} onChange={handleChange} required />
          </div>
          <div>
          ...
          <Button type="submit" className="w-full bg-gradient-primary shadow-glow-primary" disabled={isLoading}>
