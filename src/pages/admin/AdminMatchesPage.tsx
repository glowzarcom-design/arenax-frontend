...
export default function AdminMatchesPage() {
  const [formData, setFormData] = useState({
    title: '',
    gameName: 'Free Fire', // Default game name
    entryFee: '',
    prizePool: '',
    maxPlayers: '',
    startTime: '',
    ...
    const { error } = await supabase.from('matches').insert([
      {
        ...
        // created_by: user.id // Agar 'created_by' column hai toh
      },
    ]);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="p-6 bg-gradient-card border-card-border lg:col-span-1">
          ...
          <form onSubmit={handleCreateTournament} className="space-y-4">
            ...
            <textarea name="description" value={formData.description} onChange={handleInputChange} placeholder="Description" className="w-full bg-input border border-border rounded-lg p-2 text-sm" />
            <textarea name="rules" value={formData.rules} onChange={handleInputChange} placeholder="Rules" className="w-full bg-input border border-border rounded-lg p-2 text-sm" />
